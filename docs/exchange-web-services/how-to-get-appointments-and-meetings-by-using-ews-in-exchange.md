...
---
# <a name="get-appointments-and-meetings-by-using-ews-in-exchange"></a>Créer des rendez-vous et des réunions à l’aide d’EWS dans Exchange

Découvrez comment obtenir des rendez-vous et des réunions à l’aide de l’API managée EWS ou d’EWS dans Exchange.
...
L’exemple de code suivant montre comment utiliser l’API managée EWS pour récupérer les rendez-vous d’un utilisateur qui se situent entre une heure de début et une heure de fin spécifiées.
...
Voici le résultat de l’exemple de code.
...
## <a name="get-appointments-by-using-ews"></a>Obtenir des rendez-vous à l’aide d’EWS
<a name="bk_xml"> </a>

Le code XML suivant montre une demande d’opération [GetFolder](https://msdn.microsoft.com/library/355bcf93-dc71-4493-b177-622afac5fdb9%28Office.15%29.aspx) permettant de renvoyer un ID de dossier pour l’opération [FindItem](https://msdn.microsoft.com/library/ebad6aae-16e7-44de-ae63-a95b24539729%28Office.15%29.aspx) . 
...
Le code XML suivant illustre la réponse **GetFolder** . Notez que les attributs **FolderId** et **ChangeKey** sont raccourcis pour des raisons de lisibilité. 
...
Le code XML suivant montre la requête **FindItem** utilisée pour renvoyer les rendez-vous demandés. Notez que les attributs **FolderId** et **ChangeKey** sont raccourcis pour des raisons de lisibilité. 
...
Le code XML suivant illustre la réponse **FindItem** . Notez que les attributs **ItemId** et **ChangeKey** sont raccourcis pour des raisons de lisibilité. 
...
## <a name="recurring-meetings-and-the-calendar-view"></a>Réunions périodiques et affichage Calendrier
<a name="bk_recurring"> </a>

Le dossier calendrier est légèrement différent des autres dossiers d’une boîte aux lettres, car les occurrences d’une série périodique et les exceptions à une série périodique ne sont pas des éléments réels dans la boîte aux lettres, mais sont plutôt stockées en tant que pièces jointes à une forme de base périodique. Cela signifie que, bien que vous puissiez créer une requête EWS qui renvoie des valeurs entre un ensemble de valeurs de **début** et de **fin** à l’aide d’une des méthodes de surcharge **FindItems** de l’API managée EWS, telles que [ExchangeService. FINDITEMS](https://msdn.microsoft.com/library/microsoft.exchange.webservices.data.exchangeservice.finditems%28v=exchg.80%29.aspx) ou l’opération EWS [FindItem](https://msdn.microsoft.com/library/ebad6aae-16e7-44de-ae63-a95b24539729%28Office.15%29.aspx) , EWS ne parcourt pas la table des pièces jointes de chaque élément de calendrier pour trouver des exceptions et des occurrences 
  
Au lieu de cela, ce que vous voulez faire est une autre façon d’appliquer un *DataView* à une Union de deux tables SQL, à l’aide d’un objet [CalendarView](https://msdn.microsoft.com/library/microsoft.exchange.webservices.data.calendarview%28v=exchg.80%29.aspx) . Pour des raisons de performances, nous vous recommandons d’utiliser la propriété [PropertySet](https://msdn.microsoft.com/library/microsoft.exchange.webservices.data.propertyset%28v=exchg.80%29.aspx) pour limiter la taille de la réponse en indiquant le nombre de rendez-vous ou de réunions que vous souhaitez renvoyer, ainsi que les propriétés spécifiques de votre choix. 
