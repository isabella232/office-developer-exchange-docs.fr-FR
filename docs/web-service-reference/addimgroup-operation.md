...
L’opération **AddImGroup** les services Web Exchange (EWS) ajoute un nouveau groupe de messagerie instantanée à une boîte aux lettres. 
  
Cette opération est une nouveauté d’Exchange Server 2013.
  
## <a name="using-the-addimgroup-operation"></a>Utilisation de l’opération AddImGroup

L’opération **AddImGroup** ne prend qu’un seul argument Nom complet. 
  
Cette opération renvoie le nom complet, le type de groupe et l’identificateur de la Banque d’Exchange du nouveau groupe.
...
## <a name="addimgroup-operation-request-example-create-a-new-im-group"></a>Exemple de requête d’opération AddImGroup : créer un groupe de messagerie instantanée

L’exemple suivant de demande d’opération **AddImGroup** montre comment créer un groupe de messagerie instantanée nommé MyCustomerGroup. 
...
L’exemple suivant montre une réponse d’erreur à une demande d’opération **AddImGroup** . Il s’agit d’une réponse à une requête qui contient un caractère qui ne peut pas être utilisé dans un nom d’affichage. Notez qu’il s’agit d’une erreur SOAP et non d’un message d’erreur basé sur un schéma. Le nom complet soumis dans la demande est ~ ! @ # $% ^ &amp; , et l’erreur se produit sur le &amp; caractère. Le &amp; caractère s’est produit sur la onzième ligne et le caractère 33e dans la charge utile de la demande. La réponse a été renvoyée avec un code HTTP 500. 
