...
Le client construit la requête XML et l’envoie au serveur. La demande identifie qui envoie le client en tant que la boîte aux lettres pour récupérer les conseils de messagerie et les conseils de la messagerie sont demandées. Dans cet exemple, le client demande que tous les conseils de messagerie être renvoyés pour la boîte aux lettres sélectionnée.
...
L’exemple de corps Simple Object Access Protocol (SOAP) suivant montre une réponse positive à la demande **GetMailTips** . 
...
- [Les infos-courrier](mailtips.md)
