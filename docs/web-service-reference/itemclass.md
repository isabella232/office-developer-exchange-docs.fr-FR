...
Une valeur de texte qui est cohérente avec type l’élément peut être nécessaire. Par exemple, si vous créez ou un message, l’IPM de mise à jour. Remarque ou une autre classe qui correspond à un message est requise. Une valeur vide n’est pas autorisée. Mais si vous créez ou mise à jour d’un élément, un type vide est valid.
  
Une erreur est renvoyée si **ItemClass** est définie sur une valeur qui n’est pas cohérente avec le type d’élément. Par exemple, d’un message **ItemClass** ne peut avoir la valeur **ItemClass** pour une tâche. 
