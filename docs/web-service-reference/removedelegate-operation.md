...
L’exemple de code suivant montre comment supprimer deux délégués de la boîte aux lettres de l’utilisateur 1. Dans cet exemple, un délégué est supprimé à l’aide de l’adresse SMTP principale du délégué, et l’autre est supprimé à l’aide de l’identificateur de sécurité (SID) du délégué.
...
**L’opération RemoveDelegate** n’exige pas que l’utilisateur délégué spécifié soit en même temps qu’une boîte aux lettres ou qu’il existe dans le service d’annuaire Active Directory. **L’opération RemoveDelegate** réussit si l’entrée du délégué est orpheline. 
  
## <a name="removedelegate-response-example"></a>Exemple de réponse RemoveDelegate

### <a name="description"></a>Description

L’exemple suivant **d’une réponse RemoveDelegate** affiche une réponse réussie à **une demande RemoveDelegate.** La réponse contient un **élément DelegateUserResponseMessageType** pour chaque délégué supprimé de la boîte aux lettres. 
...
L’exemple suivant de réponse **d’erreur RemoveDelegate** affiche les résultats d’une demande de suppression d’un délégué qui n’existe pas. 
